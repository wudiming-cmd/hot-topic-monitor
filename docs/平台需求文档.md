...
### 4.1 实时监控页(✅ 已实现)
- **综合榜 + 分平台榜**:平台 Tab(综合/YouTube/Reddit/Hacker News/Google News/TMDb/Giphy/TikTok/Facebook)。
  - 注:TikTok / Facebook 已在前端就绪;真实数据 TikTok 需第三方数据商,Facebook 自然内容无官方热榜 API(公开数据以广告资料库为主),详见 §6.2。
...
- **TikTok**(前端已就绪,标签/筛选可用):推荐数据源为**官方 TikTok Creative Center**(ads.tiktok.com/creative/creativeCenter → Trends),免费免登录,提供热门话题标签/音乐/视频/Top Ads,可按地区/行业/时间筛选。后端接入两种方式:① 复刻网页的匿名令牌握手后调用其 `creative_radar_api/...` JSON 接口;② 用无头浏览器(Playwright)渲染 Trends 页抓取。均需遵守 ToS 与频率限制。(Display API=仅自有授权内容、Research API=仅学术机构,不适用于此场景;第三方数据商为备选。)
- **Facebook**(前端已就绪,标签/筛选可用):自然内容无官方"热榜"API,FB 侧公开可用数据**以广告资料库为主**(见上)。
- Tenor、Imgflip(梗图,免费/免 key)。
- X/Twitter(API v2,按量付费,慎用)、Pinterest。
